Module ModuleMCCSigma
    ...
    contains
        ...
        subroutine  SigmaNormalization(SN,SO,GO,Inited)
            ...
            !GO%Name="Ar"
            Filename="./input/gas/lxcat/"//Trim(GO%Name)//"/"//Trim(SO%Name)//Trim(GO%Name)//".txt"
            ...
            If(Inited) then
                ...
                DeAllocate(SR)
            Else
                if (0 == rank) write(*, *) "The file ", trim(Filename), " is not found."
            End If
            return
        End  subroutine
   ...
   Subroutine  SigmaNormalizedInit(SN,Model,NReaction)
    ...
    Subroutine SigmaRawFinalization(SR)
        ...
        NSigmaRaw=NSigmaRawLoc(1)
        call tmp_name%Init("SR_"//trim(num2str(Timer, 3)), &
    ...
    Subroutine SigmaNormalizedFinalization(SN)
        ...
        Timer=Timer+1
        Associate(Emin=>SN%EnergyMin,Eint=>SN%EnergyInterval,Nr=>SN%NReaction,Ns=>SN%NSigma)
            call tmp_name%Init("SN_"//trim(num2str(Timer, 3)), &
                                PathMode=FILE_PATH_MODE_CHECK, &
                                ExtensionMode=FILE_EXTENSION_MODE_DAT, &
                                ParallelMode=FILE_PARALLEL_MODE_CLOSE, &
                                DynamicIndex=FILE_DYNAMIC_MODE_CLOSE)
